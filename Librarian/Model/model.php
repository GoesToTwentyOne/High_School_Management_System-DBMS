...
function GetFine($uid){
    $conn = db_conn();
    $selectQuery = "SELECT * FROM `issuebook` where uid = ?";
...
function BalanceT($uid, $balance)
{
    $conn = db_conn();
    
    
    $selectQuery = "UPDATE `Teacher` SET balance = ? WHERE uid = ?";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([$balance, $uid]);
    }catch(PDOException $e){
...
function ReturnBookT($uid, $bid, $status, $id, $ret, $fineT, $balancet)
{
    $conn = db_conn();

    $updateBookQuery = "UPDATE `book` SET status = ? WHERE bid = ?";
    try {
    ...
    $updateIssueBookQuery = "UPDATE `issuebook` SET returndate = ?, fine = ? WHERE id = ?";
    try {
        $stmt = $conn->prepare($updateIssueBookQuery);
        $stmt->execute([$ret, $fineT, $id]);
    } catch (PDOException $e) {
        echo $e->getMessage();
    }

    $updateTeacherBalanceQuery = "UPDATE `teacher` SET balance = ? WHERE uid = ?";
    try {
        $stmt = $conn->prepare($updateTeacherBalanceQuery);
        $stmt->execute([$balancet, $uid]);
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
...
function obtainIssueList() {
    ...
    $query = "SELECT ib.id, cn.name
              FROM issuebook ib
              LEFT JOIN (
                  SELECT uid, name FROM student
                  UNION
                  SELECT uid, name FROM teacher
              ) AS cn ON ib.uid = cn.uid
              ORDER BY ib.id DESC
                LIMIT 1
              ";
